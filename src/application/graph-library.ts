...
const definitions: readonly GraphLibraryDefinition[] = [
  ...
  },
  {
    ...
    source: source(
      'langchain-ai',
      'open_deep_research',
      'Normalized from the pinned source: its asyncio researcher pool is represented as Send/Merge, and a mandatory first research batch is an explicit planning guardrail. Defaults of five concurrent units, six supervisor rounds, and ten researcher tool-call rounds are retained; no production Checkpointer or Store is claimed.',
    ),
    layout: {
    ...
    graph: graph(
      ...
      [
        start('research-start', 40, 500),
        step('clarify-request', 'Clarify request', 'ai', 300, 500, {
          description: 'Resolve ambiguity before creating the research brief.',
        }),
        end('awaiting-user-reply', 'Awaiting user reply', 580, 330),
        step('write-brief', 'Write research brief', 'ai', 580, 670, {
          description: 'Convert the clarified request into a focused research brief.',
        }),
        start('research-cell-start', 50, 110, 'research-cell'),
        step('frame-question', 'Plan research', 'ai', 310, 110, {
          parentId: 'research-cell',
          description: 'The Supervisor plans at least one bounded research batch.',
          participation: { internalTools: true },
          config: { sourceNode: 'supervisor', maxResearcherIterations: 6 },
        }),
        step('review-findings', 'Review findings', 'ai', 1400, 110, {
          parentId: 'research-cell',
          description: 'The Supervisor assesses merged evidence and either researches more or completes.',
          participation: { internalTools: true },
          config: { sourceNode: 'supervisor', maxResearcherIterations: 6 },
        }),
        end('research-cell-end', 'Research complete', 1660, 110, 'research-cell'),
        start('researcher-start', 50, 100, 'researcher-workflow'),
        step('dispatch-research', 'Dispatch research', 'deterministic', 310, 100, {
          parentId: 'researcher-workflow',
          description: 'Fan out independent research topics to at most five concurrent workers.',
          config: { maxConcurrentResearchUnits: 5 },
        }),
        step('researcher-agent', 'Researcher Agent', 'ai', 370, 302, {
          parentId: 'researcher-workflow',
          description: 'Canonical worker template: research with configured tools, then compress the findings.',
          participation: { internalTools: true },
        ...
        end('researcher-end', 'Researchers complete', 1270, 100, 'researcher-workflow'),
        step('final-report', 'Final report generation', 'ai', 2920, 500, {
          description: 'Synthesize the merged research notes into the final report.',
          retry: { maxAttempts: 3, backoff: { strategy: 'fixed', initialDelayMs: 0 } },
        }),
        end('research-complete', 'Report complete', 3180, 500),
      ],
      [
        ...
        { id: 'frame-researcher', source: 'frame-question', target: 'researcher-start', mode: 'command', label: 'conduct research', condition: 'supervisor.researchRequired' },
        { id: 'researcher-dispatch', source: 'researcher-start', target: 'dispatch-research', mode: 'normal' },
        {
          id: 'dispatch-send',
          ...
          send: {
            ...
            templateAnatomy: {
              ...
              nodes: [
                { id: 'researcher-worker-start', kind: 'start', label: 'Start', position: { x: 28, y: 150 }, dimensions: { width: 180, height: 100 } },
                { id: 'researcher-agent', kind: 'step', executor: 'ai', label: 'Researcher Agent', position: { x: 300, y: 132 }, dimensions: { width: 220, height: 134 } },
                { id: 'research-tools', kind: 'step', executor: 'tool', label: 'Research Tools', position: { x: 570, y: 132 }, dimensions: { width: 220, height: 134 } },
                { id: 'compress-findings', kind: 'step', executor: 'ai', label: 'Compress Findings', position: { x: 840, y: 132 }, dimensions: { width: 220, height: 134 } },
                { id: 'researcher-worker-end', kind: 'end', label: 'End', position: { x: 1150, y: 150 }, dimensions: { width: 180, height: 100 } },
              ],
              edges: [
                { id: 'worker-start-agent', source: 'researcher-worker-start', target: 'researcher-agent' },
                { id: 'worker-agent-tools', source: 'researcher-agent', target: 'research-tools' },
                { id: 'worker-tools-compress', source: 'research-tools', target: 'compress-findings' },
                { id: 'worker-compress-end', source: 'compress-findings', target: 'researcher-worker-end' },
              ],
            },
          },
        },
        { id: 'researcher-merge', source: 'researcher-agent', target: 'research-merge', mode: 'normal' },
        { id: 'merge-researcher-end', source: 'research-merge', target: 'researcher-end', mode: 'normal' },
        { id: 'researcher-supervisor-review', source: 'researcher-end', target: 'review-findings', mode: 'normal' },
        { id: 'researcher-supervisor-loop', source: 'review-findings', target: 'frame-question', mode: 'command', label: 'research more', condition: 'supervisor.researchRequired', loopCap: 5 },
        { id: 'frame-complete', source: 'review-findings', target: 'research-cell-end', mode: 'command', label: 'research complete', condition: 'supervisor.complete' },
      ...
      [
        {
          ...
          position: { x: 850, y: 50 },
          dimensions: { width: 1930, height: 1080 },
          collapsed: false,
        },
        {
      ...
      durability({
        state: { enabled: true, schema: { fields: ['researchBrief', 'researchTasks', 'researchIterations', 'researchNotes', 'rawNotes', 'finalReport'], summary: 'Per-run brief, bounded research rounds, dynamic tasks, merged notes, and final report.' }, reducers: [{ key: 'researchNotes', summary: 'Aggregate completed dynamic researcher notes' }, { key: 'rawNotes', summary: 'Append raw notes returned by researcher workers' }] },
      }),
    ),
  },
  {
