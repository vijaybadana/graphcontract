...
Dispatch waves: W0 `P0-H`; W0b `P0-V` and `P0-M`; W0c `P0-R`; W1 `P1-D`; W2 `P1-A`, `P1-C`, and `P1-W` after domain review/freeze; W3 `P1-I` after all seams integrate; W4 `P1-Q` integrates the protected Playwright library at the first clean runnable Package 1 boundary; Lead closes the full package and browser gate.
...
Scenario coverage: subgraph entry/exit without React Flow warnings → `P0-H` → mounted projection proof; self/duplicate topology rejected canonically and source-scoped invalid routes projected → `P0-V` → domain/proposal/WebMCP/projection proof; incomplete drafts reload without sample replacement → `P0-M` → persistence migration proof; route-mode switches normalize incompatible data and bounded-cycle wording remains truthful → `P0-R` → inspector/export/docs proof; compact draft/frozen controls retain status-aware accessible names → `P0-R` → mounted workspace accessibility proof; legacy Agent/Action/Tool/Human migration and schema-v2 round trip → `P1-D` → domain/migration proof; preset creation, undo/redo, persistence and atomic proposals → `P1-A` → application/store proof; Step anatomy, modifier combinations, overflow and interaction states → `P1-C` → projection/DOM proof; WebMCP normalized Step proposals → `P1-W` → adapter/application proof; inspector editing, keyboard overflow, frozen/proposal coexistence and real journey → `P1-I` → mounted integration proof; demo/routing/edit/freeze/persistence/scenarios/download/WebMCP/compact/console journeys → `P1-Q` → full cold Chromium proof.
...
- Compact human-authority controls retain explicit names → production path: workspace freeze/unfreeze action across responsive presentation → prohibited failure: hidden visible text producing an unnamed accessibility-tree button → package: `P0-R` → proof: draft/frozen mounted accessibility checks.
...
- Package 1 retains protected user journeys in a real browser → production path: cold production build through Chromium acceptance library → prohibited failure: stale locators weakening behavior, unnamed compact authority control, download/WebMCP regression, or unexpected console failure → package: `P1-Q` → proof: discovered and complete Playwright suite.
...
- `P0-R` — normalize route-mode data, align authoritative bounded-cycle language, and close compact freeze accessibility; owner: embedded Backend Engineer; model: `gpt-5.6-terra`; effort: High; depends on: `P0-V`, `P0-M`
  ...
  - Method: define mode-compatible semantic fields and atomically remove/normalize stale incompatible data on mutation/migration; preserve fallback role semantics; replace stale acyclic/exhaustive claims with Command and bounded deterministic-cycle behavior; give the responsive freeze/unfreeze control an explicit status-aware accessible name.
  - Boundary/isolation: routing normalization, inspector/export/migration tests, authoritative wording and focused workspace accessibility coverage; no Step schema or broad UI redesign.
...
- `P1-Q` — integrate and run the protected Playwright regression library; owner: embedded QA Engineer; model: `gpt-5.6-terra`; effort: High; depends on: `P1-I`
  - Inputs: owner commit `a958347`, clean runnable Package 1 integration, source-level compact freeze accessibility closure and repository test contract.
  - Method: inspect then cherry-pick the acceptance commit; preserve its seven journey intents; resolve only genuine normalized-Step semantic/locator changes; add the status-aware compact freeze/unfreeze accessible-name assertion.
  - Boundary/isolation: Playwright configuration, fixtures/specs, package scripts/dependency, QA docs and minimal locator alignment only; production behavior changes return to the owning package instead of being hidden in tests.
  - Produces/freeze: protected browser gate that downstream packages must search and extend before adding acceptance coverage.
  - Done/return: Playwright discovery succeeds; complete cold Chromium suite passes; lint and diff check pass; exact commit/result and any platform limitation.

Integration: Lead has reviewed and committed `P0-H` at `4f61e80`, `P0-M` at `b7ca719`, `P0-V` at `f658200`, `P0-R` at `b825174`, and schema-v2 `P1-D` at `bd8647d`; then accepts non-overlapping `P1-A`, `P1-C`, `P1-W` commits before dispatching `P1-I`; `P1-Q` integrates the protected Playwright library only after the candidate is runnable and closes the real-browser gate; final reconciliation checks canonical graph/projection separation, legacy migration meaning, human authority, exports and modifier/diff coexistence.
...
Active correction (2026-08-30): owner adversarial QA against candidate `4f61e80` demonstrated that canonical validation accepted self/duplicate connections rejected by the UI policy, invalid routing issues could lack actionable projection paths, workspace migration replaced parseable incomplete drafts, route-mode changes retained contradictory hidden fields, authoritative docs/UI still described the now-cyclic bounded scenario system as acyclic/exhaustive, and compact CSS removed the freeze/unfreeze control's only accessible name. `P0-V`, `P0-M`, and `P0-R` are material prerequisite gates added before `P1-D`; the verified `P0-H` closure remains preserved. `P0-M` closed at `b7ca719`; `P0-V` closed at `f658200`; `P0-R` now includes the independent compact accessibility regression before schema-v2 work.

Acceptance correction (2026-08-30): owner commit `a958347` supplies seven cold Chromium regression journeys. It is inspected but must not be cherry-picked into the intentionally non-runnable schema-only boundary at `bd8647d`; `P1-Q` integrates it immediately after `P1-I`, resolves only normalized-Step semantic/locator changes, strengthens compact authority naming, and establishes the required search-first acceptance library for all later packages.
